...
public class BinarySearchTree {
	...
	public TreeNode find(final Integer data) {
		if (root != null) {
	...
	public void delete(final Integer data) {
		...
		// This is case 3 - Most complicated (node to be deleted has 2 children)
			...
			final TreeNode successor = getSuccessor(current);
			if (current == root) {
				root = successor;
			} else if (isLeftChild) {
	...
	public static BinarySearchTree createFromSortedArray(final int[] data) {
	...
	public static void main(final String[] args) {
		final int[] sample = { 212, 580, 6, 7, 28, 84, 112, 434};
		final BinarySearchTree bst = new BinarySearchTree();
		for (final int x : sample) {
			bst.insert(x);
		}
